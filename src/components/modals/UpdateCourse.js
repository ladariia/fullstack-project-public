...
import { fetchTypes, fetchFormats, createCourse, fetchDocuments, fetchCourses, fetchCourse } from '../../http/courseAPI';
import { observer } from 'mobx-react-lite';

const CreateCourse = observer(({ show, onHide }) => {
    const { course } = useContext(Context)
    const [courseUpdate, setCourse] = useState({ course_module: [], course_shedule: [], format: [], type: [], document: [], subjects: [] })
    ...
    const addCourse = async () => {
        try {
            ...
            await createCourse(formData).then(data => fetchCourses(null).then(data => { course.setCourses(data) }))
            onHide()
        } catch (error) {
            alert(error.response.data.message)
        }
